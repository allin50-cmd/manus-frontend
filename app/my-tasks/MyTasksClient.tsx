...
const PERSONS = OWNERS.filter((person) => person !== 'Team' && person !== 'Unassigned')
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-xs font-medium rounded px-2 py-0.5 ${map[status] ?? 'bg-slate-100 text-slate-600'}`}
    >
      {status}
    </span>
  )
...
export default function MyTasksClient({ initialTasks, currentPerson }: Props) {
  ...
  async function markDone(task: Task) {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/work-items/${task.workItem.id}/actions/${task.id}`, {
        method: 'PATCH',
      ...
      if (!res.ok) {
        ...
        setReassigning((current) => (current === task.id ? null : current))
      }
    } catch {
  ...
  async function submitReassign(task: Task) {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/work-items/${task.workItem.id}/actions/${task.id}/reassign`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assignedTo: reassignPerson,
      ...
      if (!res.ok) {
        ...
        if (reassignPerson !== currentPerson) {
          ...
          setTasks((prev) =>
            prev.map((t) => (t.id === task.id ? { ...t, assignedTo: reassignPerson } : t)),
          )
        }
        setReassigning(null)
        setReassignPerson('')
        ...
        <div className="flex gap-1 flex-wrap">
          {dueTabs.map((tab) => (
            <button
              key={tab.key}
              type="button"
              aria-pressed={dueFilter === tab.key}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((task) => {
            ...
            const reassignmentChoices = PERSONS.filter((person) => person !== task.assignedTo)
            ...
            return (
              ...
              >
                ...
                {reassigning === task.id && !isTerminal && (
                  <div className="border-t border-slate-100 pt-3 space-y-2">
                    <p className="text-xs font-semibold text-slate-600">Reassign to:</p>
                    <div className="flex gap-2 flex-wrap">
                      {reassignmentChoices.map((person) => (
                        <button
                          key={person}
                          type="button"
                          aria-pressed={reassignPerson === person}
                          onClick={() => setReassignPerson(person)}
