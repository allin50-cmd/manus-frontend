...
export default function MorningBriefing({ items }: { items: BriefingItemClient[] }) {
  ...
  // Tile colours
  const decisionTileColor =
    decisionsNeeded.length > 0
      ? 'bg-red-50 border-red-200 text-red-700'
      : 'bg-slate-50 border-slate-200 text-slate-500'

  const overdueTileColor =
    overdue.length > 3
      ? 'bg-red-50 border-red-200 text-red-700'
      : overdue.length > 0
  ...
  const escalatedTileColor =
    escalated.length > 0
      ? 'bg-red-50 border-red-200 text-red-700'
      : 'bg-slate-50 border-slate-200 text-slate-400'

  const followUpTileColor =
    followUpDueToday.length > 0
      ? 'bg-amber-50 border-amber-200 text-amber-700'
      : 'bg-slate-50 border-slate-200 text-slate-400'
        ...
        <div className="space-y-3">
          <div id="briefing-decisions">
            <CollapsibleSection
              ...
              emptyLabel="decisions needed"
              defaultOpen={decisionsNeeded.length > 0}
            />
          </div>
          ...
          <div id="briefing-escalated">
            <CollapsibleSection
              ...
              items={escalated}
              emptyLabel="escalated items"
              defaultOpen={escalated.length > 0}
            />
          </div>
          <div id="briefing-followup">
            <CollapsibleSection
              title="Follow-Ups Due Today"
              items={followUpDueToday}
              emptyLabel="follow-ups due today"
              defaultOpen={followUpDueToday.length > 0}
            />
          </div>
        </div>
      )}
