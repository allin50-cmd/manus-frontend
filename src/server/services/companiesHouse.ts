...
export class CompaniesHouseService {
   ...
   */
  async searchCompanies(query: string): Promise<{ companyNumber: string; companyName: string; companyStatus: string }[]> {
    const url = `${CH_API_BASE}/search/companies?q=${encodeURIComponent(query)}&items_per_page=5`;
    const response = await fetch(url, { headers: this.getAuthHeaders() });
    if (!response.ok) return [];
    const data = await response.json() as { items?: { company_number: string; title: string; company_status: string }[] };
