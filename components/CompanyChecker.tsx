...
const STATUS_CFG = {
  green: {
    ...
    label: 'GREEN',
    icon: (
      <svg className="w-6 h-6 text-[#00A86B] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
  ...
  amber: {
    ...
    label: 'AMBER',
    icon: (
      <svg className="w-6 h-6 text-amber-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
  ...
  red: {
    ...
    label: 'RED',
    icon: (
      <svg className="w-6 h-6 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
    ...
    <div className="w-full max-w-2xl mx-auto">
      <form onSubmit={handleCheck} className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          autoComplete="off"
          spellCheck={false}
          className="flex-1 px-5 py-4 rounded-xl text-slate-900 text-lg font-mono bg-white border-0 focus:outline-none focus:ring-2 focus:ring-[#00A86B] shadow-xl placeholder:text-slate-400 placeholder:font-sans"
        />
        <button
          type="submit"
      ...
      {result && cfg && (
        <div className={`mt-6 p-6 rounded-2xl border-l-4 text-left shadow-xl ${cfg.bg} ${cfg.border}`}>
          ...
          <p className="text-slate-700 text-base leading-relaxed">{result.message}</p>
          {result.status !== 'red' ? (
            <div className="mt-4 pt-4 border-t border-slate-200">
              <p className="text-sm text-slate-500">
                FineGuard can monitor this company and alert you before any deadline.{' '}
                <a href="#pricing" className="text-[#00A86B] font-semibold hover:underline">
                  See plans →
                </a>
              </p>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-red-200">
              <p className="text-sm text-red-700">
                ...
                <a href="mailto:help@fineguard.co.uk" className="font-semibold hover:underline">
                  Contact us now →
                </a>
              </p>
            </div>
