  ...
  return (
    <div className="space-y-10">
      <section className="grid gap-8 lg:grid-cols-2">
        <form onSubmit={process} className="space-y-5 rounded-xl border border-gray-200 bg-white p-6">
          ...
          <Field label="Rate per hour (£)">
            <input type="number" min={0} value={rate} onChange={(e) => setRate(Number(e.target.value))} className={inputCls} />
          </Field>
          <button disabled={processing} className="rounded bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-black disabled:opacity-50">
            ...
            <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-6">
              ...
              {processResult.parties && processResult.parties.length > 0 && (
                <Section title="Parties">
                  <p className="text-sm">{processResult.parties.join(', ')}</p>
                </Section>
              )}
                ...
                <Section title={`Billing entries (${processResult.billingEntries.length})`}>
                  <table className="w-full text-sm">
                    <thead className="text-left text-xs uppercase text-gray-500">
                      <tr><th className="py-1">Description</th><th>Hours</th><th>Value</th></tr>
                    </thead>
                    <tbody>
                      {processResult.billingEntries.map((e, i) => (
                        <tr key={i} className="border-t border-gray-100">
                          <td className="py-1 pr-4">{e.description}</td>
                          <td className="py-1 pr-4">{e.hours}</td>
      ...
      <section className="grid gap-8 lg:grid-cols-2">
        <form onSubmit={genBilling} className="space-y-5 rounded-xl border border-gray-200 bg-white p-6">
          ...
          <Field label="Rate per hour (£)">
            <input type="number" min={0} value={billRate} onChange={(e) => setBillRate(Number(e.target.value))} className={inputCls} />
          </Field>
          <button disabled={billing} className="rounded bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-black disabled:opacity-50">
